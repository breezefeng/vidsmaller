...
### 3.1 编码 preset：同一个文件跑三遍（2026-09-06）

素材 `20260327-154029.mp4`：720p，507.6s，54,471,871 B，libx264，`by_percentage: 50`。
原始数据 `scripts/fc-speed-preset-results.jsonl`，job id 可回查。

| speed | compress | 相对 medium | 输出字节 | 本地 VMAF\* |
|---|---|---|---|---|
| medium | 61.68s | 1.00x | 25,703,118 | 96.06 |
| fast | 43.23s | 0.70x | 25,702,451 | 96.05 |
| faster | 27.33s | **0.44x** | 25,701,649 | 95.96 |

\* 本地 ffmpeg，固定码率 360 kbps，60s 720p 切片，libvmaf 对原片打分。

三个输出大小差 **0.006%**——按百分比压缩时，preset 换来的是墙上时间，不是体积；
代价只有画质，而 0.1 个 VMAF 点远在可见阈值以下（JND 约 1–2 分）。

于是默认 speed 从 `medium` 改成 `faster`：**同样的文件大小，编码时间减半，
上游账单也减半**（provider 按墙上时间计费）。`medium` / `slow` 仍在高级选项里。

### 3.2 分辨率系数：按码率猜是错的

上面这条 720p 数据也顺手证伪了旧的分辨率推断：`providerResolutionFactor()`
按码率把低码率 720p 判成 `1/2.25`，而实测是

```
(61.68 - 6.1) / (0.1191 × 507.6) = 0.92
```

即 720p 每秒源视频的成本是 1080p 的 **92%**，不是 44%——provider 的流水线并非
纯编码瓶颈。低估两倍，而且恰好低估在用户最常上传的那类文件上（已压过的 720p 片源）。
浏览器现在会把宽高一起报上来，`estimateProviderMinutes()` 优先用真实高度，
拿不到时才退回码率推断。

### 3.3 真实任务的成本，以及为什么估算不能当账单

§3 那条拟合是拿 `testsrc2` 生成的素材测的，**真实文件更贵**。用真实任务重测
（`scripts/fc-real-job-costs.jsonl`，job id 可回查），全部 720p：

| 素材 | 时长 | 大小 | 源码率 | speed | compress | 每秒源视频 |
|---|---|---|---|---|---|---|
| 短片 | 8.5 min | 54 MB | 0.86 Mbps | faster | 16.5 / 19.4 / 27.3 / 44.3s | 0.033–0.087 |
| 长片 A | 80 min | 363 MB | 0.63 Mbps | medium | 504.1 / 279.7s | 0.058–0.105 |
| 长片 A | 80 min | 363 MB | 0.63 Mbps | faster | 350.6 / 306.3s | 0.064–0.073 |
| 长片 B | 46 min | 702 MB | 2.12 Mbps | faster | 385.4s | **0.139** |

拟合模型对这三种素材一律给 0.048 s/s，也就是**真实内容是合成素材的 0.85–2.9 倍，
中位数 1.33 倍**——这就是 `PROVIDER_REAL_CONTENT_FACTOR`。

两个必须记住的结论：

1. **离散度压过了一切参数。** 同一个文件同一组设置测到过 16.5s 和 44.3s；长片在
   `medium` 下是 504s / 280s，在 `faster` 下是 351s / 306s——preset 的差别在长任务
   上直接被机器差异吞掉了（短片上仍然稳定 2.3 倍，见 §3.1 的背靠背对照）。
2. **export 不是免费的。** 它把成品写进我们自己的桶，耗时随**输出**大小走：
   337 MB → 61.1s（自己就吃掉 2 个计费分钟），178 MB → 6.6/11.6/18.2s，25 MB → <1s。
   旧模型一律按 1 分钟算。

### 3.4 于是：估算是预扣，账单以 provider 的表为准

一个 702 MB / 46 分钟的真实任务**扣了用户 5 个 credit，实际计费 10 分钟**。
这不是模型调参能修的——单次任务的误差本来就有 2–3 倍，而且是**单边**的：
落到慢机器上的那次我们全赔，落到快机器上的那次用户也不会退钱。

改法（`lib/compress/service.ts`）：

- 创建任务时按**保守值**预扣（`PROVIDER_ESTIMATE_HEADROOM`，压在离散区间上沿）；
- 任务进终态时，用 provider 自己的 `max(1, ceil(task_seconds/60))` 求和作为账单，
  多退少补：退款是常态，补扣以余额为上限，且**永远不因此让任务失败**——文件已经
  编码完了，上游那一分钟已经花掉了；
- 这一步挂在 `provider_billed_minutes` 由 null 变成数字的那次 UPDATE 上，
  谁抢到行谁结算，轮询和 webhook 并发也只会结一次。

拿今天 12 个真实任务回放：预扣合计 99 分钟，实际计费 77 分钟，结算后用户付的正好
是 77——其中 10 个任务预扣够用（退还差额），2 个不够（从余额补扣）。
...
## 7.1 进度条：provider 不给百分比

2026-09-06 实测：compress task 在整个编码过程中只有 `status: "processing"`，
**没有 percent、没有帧数、没有已写字节**。所以旧的「三个 task 等权重」算法
必然把整段编码显示成一个固定的 50%——90 分钟的片子就是十分钟不动的 50%。

改成按时钟插值：

- 服务端从 provider 负载里读出当前 stage（import / compress / export）和它的
  `startedAt`，加上用 §3 模型预测的该 stage 时长，一起下发给浏览器；
- 浏览器每秒用同一条曲线重算（`lib/compress/progress.ts` 前后端共用），
  所以两次轮询之间进度条也在动，ETA 在倒数；
- 曲线在预测时间点走到该 stage 的 88%，之后渐近爬升但永远到不了 100%——
  超时的任务会变慢，不会撒谎说已经完成。

预测精度（`lib/compress/eta.ts`，对照全部 6 次实测）：1080p/720p、x264/x265、
medium/fast 四组误差都在 ±5% 内，faster 偏保守 +12%（ETA 宁可报长）。

不过 ETA 的天花板不在模型：**同一个文件同一组参数跑四遍，compress 分别是
16.5s / 19.4s / 27.3s / ~45s**——provider 把任务分到快慢明显不同的机器上，
2.7 倍离散是常态。所以预测只对准这段离散的中位，曲线的设计目标是两个方向
都能兜住：提前完成就直接跳 100%，超时就变慢继续爬。
...
| credit 改为按实际计费结算 | `service.ts` | 5 credit / 10 分钟的单边亏损归零 |
| export 按输出大小估算 | `config/compress.ts` | 700 MB 输出不再少算 1 分钟 |
| 真实内容系数 1.35 | `config/compress.ts` | 合成素材拟合值对真实文件低估 |

预估器已与实测对齐：59 MB/1min 估 3 实测 3；593 MB/10min 估 4 实测 4。
单次任务的估算误差仍有 2–3 倍（§3.3），所以钱的正确性由 §3.4 的结算兜底，不靠估准。
