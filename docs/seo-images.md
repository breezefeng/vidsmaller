...
| v1 提的图 | 状态 |
| --- | --- |
| CRF → 体积 / 画质曲线 | ✅ **已补测**。`scripts/crf-sweep.mjs`，18 次本地编码（CRF 18/20/23/26/28/32 × 实拍 / 动画 / 录屏），带 VMAF + SSIM。见 `CrfCurveChart` |
| 9 个在线压缩器输出体积对比 | ❌ **仍无数据**。同一源文件手动跑 VEED / Clideo / FreeConvert / Kapwing，记录体积、耗时、水印。这一步没法自动化 |

**CRF 扫描为什么在本地跑而不是走 provider**：CRF 曲线是 libx264 的属性，
不是托管方的属性，我们的服务和这个脚本喂给编码器的是同一批参数。走 provider
每个数据点要 3 operations + ~3 计费分钟，6×3 的网格 = 54 个，免费档要跑六天，
而且换不来任何额外真相。本地零成本，所以能负担密集采样和真实 VMAF。
代价是它是我们输出的**代理**而非录像——图上写明了 libx264 而不是 VidSmaller。

编这两张图等于把「有实测数字」这个定位自己填了，**比不做更糟**。

竞品那一轮做完能喂榜单文章（第 3 篇）；CRF 这一轮已经喂得动 CRF 原理文
（第 6 篇）和 benchmark 文章（第 4 篇）。

**这批数据最值钱的一个发现**：三类内容的曲线形状完全不同。录屏在 CRF 32
（0.79 MB, VMAF 89.9）仍在透明度线附近，而实拍在同一档已经跌到 VMAF 74.4
且体积是它的 3 倍。「能压掉多少」根本没有单一答案——这就是为什么我们的
FAQ 说「取决于源文件」，现在有数字撑着了。

#### 这两份数据还**不包含**什么

写在各自模块的文件头，避免下一个人在上面盖一张它撑不起的图。

`lib/seo/benchmark.ts`（N=3，provider 实测）：没有 CRF sweep、没有画质指标、
没有竞品数据。

`lib/seo/crf.ts`（N=18，本地实测）：每类只有一个片段、10 秒、全是 1080p——
够说明三类内容位置完全不同，不够给任何单个数字标公差。只有 H.264，没有
H.265 扫描。仍然没有竞品数据。
