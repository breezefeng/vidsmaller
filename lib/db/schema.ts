...
export type CompressionStatus =
  (typeof compressionStatusEnum.enumValues)[number]
...
export const compressionJobs = pgTable(
  'compression_jobs',
  {
    ...
    // null for signed-out visitors (tracked by anonKey instead)
    userId: uuid('user_id').references(() => user.id, { onDelete: 'cascade' }),
    /** hashed ip + ua for anonymous rate limiting / ownership */
    anonKey: text('anon_key'),

    providerJobId: text('provider_job_id').notNull(),
    provider: text('provider').default('freeconvert').notNull(),

    status: compressionStatusEnum('status')
    ...
    inputSize: numeric('input_size', { precision: 20, scale: 0 }).notNull(),
    outputSize: numeric('output_size', { precision: 20, scale: 0 }),
  ...
  (table) => {
    return {
      providerJobIdx: unique('compression_jobs_provider_job_id_unique').on(
        table.providerJobId
      ),
      userIdx: index('idx_compression_jobs_user_id').on(table.userId),
      anonIdx: index('idx_compression_jobs_anon_key').on(table.anonKey),
      statusIdx: index('idx_compression_jobs_status').on(table.status),
      createdIdx: index('idx_compression_jobs_created_at').on(table.createdAt),
    }
  }
)

export type CompressionJob = typeof compressionJobs.$inferSelect
export type NewCompressionJob = typeof compressionJobs.$inferInsert
